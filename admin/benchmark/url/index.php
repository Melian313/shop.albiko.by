...
$additionalParams = 'structure_id=' . intval(Core_Array::getGet('structure_id', 0));
...
if($iStructureId)
{
	$oAdmin_Form_Dataset
		...
		->addCondition(
			...
			array('where' =>
				array('structures.id', '=', intval($iStructureId))
		))
...
$oBenchmark_Url
	...
	->select(
		...
		//array('benchmark_urls.datetime', 'date_avr')
		//array('UNIX_TIMESTAMP(benchmark_urls.datetime)', 'date_avr')
...
{
	//echo '<br />' .  date('Y-m-d H:i:s', $iTmpTimestamp) . '  ' . $iTmpTimestamp;
	//$aTmp[date('j', $iTmpTimestamp)] = 0;
...
{
	//$aTmp[strtotime(date('Y-m-d 00:00:00', $oBenchmark_Url->date_avr))] = $oBenchmark_Url;
...
{
	//$aTitlesPerDay[] = "'" . $oBenchmark_Url->date_avr . "T23:59:59'";
	$aTitlesPerDay[] = $key*1000; //($key + 2*60*60)*1000 ; //$oBenchmark_Url->date_avr;
...
$oBenchmark_Url
	...
	->select(
		...
		array('SUM(dns_lookup) / COUNT(*) / 1000', 'dns_lookup_avr'),
		array('SUM(connect_server) / COUNT(*) / 1000', 'connect_server_avr')
		//array('HOUR(benchmark_urls.datetime)', 'time_avr')
		//array('UNIX_TIMESTAMP(benchmark_urls.datetime)', 'time_avr')
	)
	->join('structures', 'benchmark_urls.structure_id', '=', 'structures.id')
	->where('structures.site_id', '=', CURRENT_SITE)
	->where('benchmark_urls.datetime', '>=', date('Y-m-d H:59:59', $iOneDayAgo))
	//->where('benchmark_urls.time', '>', date('H:59:59', $iOneDayAgo))
...
{
	//$aTitlesPerHour[] = "'{$key}'";
	$aTitlesPerHour[] = $key * 1000; //($key + 2*60*60)*1000 ; //$oBenchmark_Url->date_avr;
