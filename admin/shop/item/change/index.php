...
$oMainTab
	->add(Admin_Form_Entity::factory('Div')->class('row')
		->add(Admin_Form_Entity::factory('Radiogroup')
			...
			->ico(array('fa-plus'))
			->name('type_of_change')
			->divAttr(array('class' => 'form-group col-lg-3 col-md-3 col-sm-3 col-xs-7')))
...
if ($oAdmin_Form_Controller->getAction() == 'do_accept_new_price')
{
	if (!$oUser->read_only)
	{
		...
		do {
			...
			{
				applySettings($oUser, $oShopItem, $increase_price_rate, $multiply_price_rate, $iDiscountID);
				...
				if(!is_null(Core_Array::getPost('flag_include_modifications')))
				{
					...
					{
						applySettings($oUser, $oShopItemModification, $increase_price_rate, $multiply_price_rate, $iDiscountID);
					}
				}
			}
