...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/form">
		...
		<h1 class="item_title"><xsl:value-of disable-output-escaping="yes" select="name" /></h1>
					...
					<form name="form{@id}" id="form{@id}" class="form-horizontal validate" action="./" method="post" enctype="multipart/form-data" role="form">
						...
						<div class="form-group">
							...
							<div class="col-xs-9 col-sm-9 col-md-9 col-lg-9">
								<div class="actions">
									<button class="button btn-cart" type="submit" name="{button_name}" value="{button_value}">
	...
	<xsl:template match="form_field">
		...
		<xsl:if test="type != 7 and type != 8">
			<div class="form-group">
				<label for="{name}" class="col-sm-3 control-label"><xsl:value-of disable-output-escaping="yes" select="caption" />
